...
import com.example.couple.data.data.Paired
import com.example.couple.data.data.UserProfile
import com.example.couple.data.data.getUser
...
class MyRelationshipFragment :Fragment(){
    ...
    private fun pairWithAnother() {
        binding.myRelationshipEnterInviteCodeCard.myRelationshipEnterCodeCard.myRelationshipEnterButton.setOnClickListener {
            val inputCode = binding.myRelationshipEnterInviteCodeCard.myRelationshipEnterCodeCard.myRelationshipPartnerCodeInput.text.toString().trim()
            ...
            query.get().addOnSuccessListener { querySnapshot ->
                for (document in querySnapshot) {
                    ...
                    builder.setPositiveButton("Yes") { dialog, which ->
                        ...
                        if (currentUserProfile != null) {
                            val currentUserProfileWithPairStatus = currentUserProfile.copy(paired = true)
                            db.collection("User").document(currentUserProfile.userId).set(currentUserProfileWithPairStatus)
                        }
                        val partnerProfile = UserProfile(userId, email, userName, imageUrl, inviteCode, paired)
                        val partnerProfileWithPairStatus = partnerProfile.copy(paired = true)
                        db.collection("User").document(userId).set(partnerProfileWithPairStatus)
                        ...
                        db.collection("Paired").add(
                            ...
                            Toast.makeText(context, "Pairing failed: ${exception.message}", Toast.LENGTH_SHORT).show()
                        }
                        dialog.dismiss()
                    }
                ...
                Toast.makeText(context, "Cannot find user: ${exception.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
